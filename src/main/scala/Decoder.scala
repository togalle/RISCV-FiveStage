...
/** This module is mostly done, but you will have to fill in the blanks in
  * opcodeMap. You may want to add more signals to be decoded in this module
  * depending on your design if you so desire.
  *
  * In the "classic" 5 stage decoder signals such as op1select and immType are
  * not included, however I have added them to my design, and similarily you
  * might find it useful to add more
  */
class Decoder() extends Module {
  ...
  /** In scala we sometimes (ab)use the `->` operator to create tuples. The
    * reason for this is that it serves as convenient sugar to make maps.
    *
  ...
  val NOP =
    List(N, N, N, N, N, branchType.DC, rs1, rs2, ImmFormat.DC, ALUOps.DC)

  val decodedControlSignals =
    ListLookup(io.instruction.asUInt(), NOP, opcodeMap)
