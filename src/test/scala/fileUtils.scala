...
object fileUtils {
  ...
  def say(
    ...
    println(
      Console.YELLOW + s"[${fname}: ${sourcecode.Line()}]" + Console.RESET + s" - $word"
    )
  }
  ...
  def sayRed(
    ...
    val fname = filename.value.split("/").last
    println(
      Console.YELLOW + s"[${fname}: ${sourcecode.Line()}]" + Console.RED + s" - $word"
    )
  }
  def sayGreen(
      word: Any
    ...
    val fname = filename.value.split("/").last
    println(
      Console.YELLOW + s"[${fname}: ${sourcecode.Line()}]" + Console.GREEN + s" - $word"
    )
  }
  ...
  def getAllTestNames: List[String] =
    getAllTests.map(_.toString.split("/").takeRight(1).mkString)

  // Not tested.
  def getAllWindowsTestNames: List[String] =
    getAllTests.map(_.toString.split("\\\\").takeRight(1).mkString)
  ...
  def readTest(testOptions: TestOptions): Either[String, List[String]] = {
    ...
    getAllTests
      .filter(_.getName.contains(testOptions.testName))
      .headOption
      .toRight(s"File not found: ${testOptions.testName}")
      .flatMap { filename =>
