...
object LogParser {
  ...
  /** Peeks ahead at the chisel log to see if an expected jump is taken.
    */
  def fetchUntilJumpTarget(
      addr: Addr,
      chiselLog: List[CircuitTrace]
  ): Option[List[CircuitTrace]] = {
    val (head, tail) = chiselLog.splitAt(4) // very arbitrary choice
    val pruned: List[CircuitTrace] = head.dropWhile { case (myAddr, _) =>
      myAddr != addr
    }
  ...
  /** Fetches a basic block of VM execution trace
    */
  def splitToBlocks(
      vmTrace: List[ExecutionTraceEvent]
  ): List[List[ExecutionTraceEvent]] =
  ...
  /** Fetches a basic block of chisel execution trace
    */
  def splitToBlocksChisel(
      chiselTrace: List[CircuitTrace]
  ): List[List[CircuitTrace]] =
    chiselTrace.splitAtPred { case ((current, _), (next, _)) =>
      !((next == current + Addr(4)) || (next == current))
    }

  def guessBlockName(trace: List[Addr], labelMap: Map[Addr, Label]): String =
    trace.headOption
      .map(x =>
        labelMap
          .lift(x)
          .getOrElse(
            labelMap
              .lift(trace.head - Addr(4))
              .getOrElse("UNKNOWN (return jump or misjump)")
          )
      )
      .getOrElse("UNKNOWN")

  /** Attempts to merge blocks, patching up when mismatches occur Fails when
    * branchpredictor misjumps, feel free to send a PR
    */
  ...
  def mergeTraces(
    ...
    def helper(
        acc: BlockList,
        blocs: (List[List[ExecutionTraceEvent]], List[List[CircuitTrace]])
    ): BlockList = blocs match {

      case (vmBlock :: vmTail, chiselBlock :: chiselTail)
          if (vmBlock.head.pc == chiselBlock.head._1) =>
  ...
  def compareRegs(
    ...
    val chiselRegUpdates = chiselTrace.zipWithIndex
      ...
      .collect { case (step: Int, addr: Addr, x: ChiselRegEvent) =>
        (step, addr, x)
      }

    val errors = (vmRegUpdates zip chiselRegUpdates).map {
      case ((_, _, vmUpdate), (_, _, chiselUpdate))
          if ((vmUpdate.reg == chiselUpdate.reg) && (vmUpdate.word == chiselUpdate.word)) => {
        None
      }

      case (
            (vmStep, vmAddr, vmUpdate),
            (chiselStep, chiselAddr, chiselUpdate)
          ) => {
    ...
    val lengthMismatch: Option[String] =
      (vmRegUpdates, chiselRegUpdates) match {
        case (h :: t, Nil) =>
          Some(
            s"Your design performed no reg updates. First expected update was at VM step ${h._1}, PC: ${h._2.show}, ${h._3.show}"
          )
        case (Nil, h :: t) =>
          Some(
            s"Your design performed reg updates, but the VM did not. First update was at step ${h._1}, PC: ${h._2.show}, ${h._3.show}"
          )
  ...
  def compareMem(
    ...
    val chiselMemUpdates = chiselTrace.zipWithIndex
      .flatMap { case (e, step) => e._2.map(y => (step, e._1, y)) }
      .collect { case (step: Int, addr: Addr, x: ChiselMemWriteEvent) =>
        (step, addr, x)
      }

    val error = (vmMemUpdates zip chiselMemUpdates)
      .map {
        case ((_, _, vmUpdate), (_, _, chiselUpdate))
            if ((vmUpdate.addr == chiselUpdate.memAddr) && (vmUpdate.word == chiselUpdate.word)) =>
          None
        case (
              (vmStep, vmAddr, vmUpdate),
              (chiselStep, chiselAddr, chiselUpdate)
    ...
    val lengthMismatch = (vmMemUpdates, chiselMemUpdates) match {
      case (h :: t, Nil) =>
        Some(
          s"Your design performed no mem updates. First expected update was at VM step ${h._1}, PC: ${h._2}, ${h._3}"
        )
      case (Nil, h :: t) =>
        Some(
          s"Your design performed mem updates, but the VM did not. First spurious update was at step ${h._1}, PC: ${h._2}, ${h._3}"
        )
      case (hVM :: tVM, hC :: tC) if (tVM.size > tC.size) => {
