...
object assembler {
  ...
  /** Will only be called by applyImmedate, thus error propagation is not needed
    ...
    */
  def setField(firstBit: Int, size: Int, field: Int): Int => Int =
    instruction => {
  ...
  /** Splits the immediate value into fields given by points. The order of
    * points is important! points is of type idx, size
    */
  def applyImmediate(
      immediateBits: Int,
      immediate: Int,
  ...
  def applyImmediateU(
      ...
      points: List[(Int, Int)],
      addr: Addr
  ): Int => InstructionFragment = instruction => {
    def totalBits = points.foldLeft(0) { case (acc, (first, size)) =>
      acc + size
    }
    totalBits.nBitsU
      .toRight("Negative number used as unsigned immediate", addr)
      .flatMap { bits =>
        Either.cond(
          bits < totalBits,
          applyImmediate(totalBits, immediate, points)(instruction),
          ("Immediate unsigned too large", addr)
        )
      }
  }
  ...
  def applyImmediateS(
    ...
    if (totalBits < immediate.nBitsS)
      Left(
        (
          s"Immediate signed too large. immedate: $immediate, immediate.nBitsS = ${immediate.nBitsS}, total bits: $totalBits",
          addr
        )
      )
  ...
  /** Used by JALR, LW and arithmetic immediate ops. JALR is sort of the odd man
    * out here as it should be unsigned. This issue should not surface at the
    * very limited address space for your design. (I hope)
    */
  def setItypeImmediate(
      immediate: Int,
      addr: Addr
  ): Int => InstructionFragment = {
  ...
  /** Used by Branches. PC relative jump, thus signed Last bit is not used,
    * hence the shift
    */
  def setBtypeImmediate(
      immediate: Int,
      addr: Addr
  ): Int => InstructionFragment = {
  ...
  /** Used by JAL. PC relative jump, thus signed The last bit is not used, hence
    * the shift
    */
  def setJtypeImmediate(
      immediate: Int,
      addr: Addr
  ): Int => InstructionFragment = {
  ...
  def setOpCode(op: Op): Int => Int = op match {
    ...
    case DONE             =>
      setOpCode("0010011".binary) // done is turned into a NOP in the assembler.
  }
  ...
  def setBranchDestination(
    ...
    labelMap
      .lift(op.dst)
      .toRight((s"destination ${op.dst} not found", opAddr))
      .flatMap { dstAddr =>
  ...
  def assembleRegLayout(op: RegLayout): Int => Int = {
    ...
    def assembleSType(op: SType): Int => Int = { instruction =>
      (setRs1(op.rs1.value) andThen
  ...
  def assembleImmediate(
      op: Op,
      addr: Addr,
      labelMap: Map[Label, Addr]
  ): Int => Either[(String, Addr), Int] = op match {
    ...
    case op: JALR          =>
      instruction =>
        labelMap
          .lift(op.dst)
          .toRight(s"label ${op.dst} not found", addr)
          .flatMap(addr => setItypeImmediate(addr.value, addr)(instruction))
  ...
  def assembleOp(
    ...
    val extras: Int => Int = (instruction: Int) =>
      op match {
        ...
        case DONE              =>
          (setFunct3("000".binary) andThen setFunct7("0000000".binary))(
            instruction
          )
