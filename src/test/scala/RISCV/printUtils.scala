...
object PrintUtils {
  ...
  implicit class FancyOps[-A](a: A)(implicit ev: Fancy[A]) {
    def show: fansi.Str = ev.s(a)
  }
  ...
  implicit class ExtraFancy(a: fansi.Str) {
    def leftPad(length: Int): fansi.Str =
      Str((0 until length).map(_ => ' ').mkString) ++ a
    def replace(from: Char, to: Char): fansi.Str =
      fansi.Str(a.toString.replace(from, to))
    def padTo(length: Int, pad: Char = ' '): fansi.Str =
      a ++ (0 until length - a.length).map(_ => pad).mkString
    def trim: fansi.Str = fansi.Str(a.toString.trim)
  }

  implicit val RegFancy = new Fancy[Reg] {
    def s(r: Reg) =
      fansi.Str(regNames.canonicalName.lift(r.value).getOrElse("ERROR"))
  }
  ...
  implicit val ExecutionEventFancy = new Fancy[ExecutionEvent] {
    def s(a: ExecutionEvent): fansi.Str = a match {
      ...
      case MemWrite(addr, word) =>
        fansi.Color.Blue(s"M[${addr.show}] <- 0x${word.hs}")
      case MemRead(addr, word) =>
        fansi.Color.Red(f"M[${addr.show}] -> 0x${word.hs}")

      // addr is the target address
      case PcUpdateJALR(addr) =>
        fansi.Color.Green(s"PC updated to ${addr.show} via JALR")
      case PcUpdateJAL(addr) =>
        fansi.Color.Magenta(s"PC updated to ${addr.show} via JAL")
      case PcUpdateBranch(from, to) =>
        fansi.Color.Yellow(s"PC updated to ${to.show} via Branch")
      case PcUpdateNoBranch(addr) =>
        fansi.Color.Yellow(s"PC updated to ${addr.show}, skipping a Branch")
    }
  }
  ...
  implicit val ExecutionTraceEventFancy = new Fancy[ExecutionTraceEvent] {
    def s(a: ExecutionTraceEvent): fansi.Str = a.event.toList match {
      case (h: ExecutionEvent) :: (t: RegUpdate) :: Nil =>
        t.show.padTo(25) ++ h.show
  ...
  implicit val ChiselEventFancy = new Fancy[ChiselEvent] {
    def s(e: ChiselEvent) = e match {
      case ChiselRegEvent(addr, reg, word) =>
        fansi.Str(s"R(${reg.show}) <- ${word.hs}")
      case ChiselMemWriteEvent(addr, memAddr, word) =>
        fansi.Str(s"M[${memAddr.show}] <- ${word.hs}")
    }
  }
    ...
    def s(e: Regs) = {
      (0 to 9)
        .map { idx =>
          ...
          cols
            .map { reg =>
              (fansi.Color.Yellow(Reg(reg).show.padTo(5)) ++ Str(
                e.repr.lift(Reg(reg)).getOrElse(0).hs
              )).padTo(16)
            }
            .showN("| ")
        }
  ...
  def printInstruction(op: Ops.Op, labelMap: Map[Label, Addr]): fansi.Str =
    op match {
      case op: Branch =>
        fansi.Color.Red(
          s"B${op.comp}\t${op.rs1.show}, ${op.rs2.show}, ${op.dst.show}\t[${labelMap.lift(op.dst).getOrElse(UNKNOWN)}]"
        )
      case op: Arith =>
        s"${op.op}\t${op.rd.show}, ${op.rs1.show}, ${op.rs2.show}"
      case op: ArithImm =>
        s"${op.op}I\t${op.rd.show}, ${op.rs1.show}, ${op.imm.show}"
      case op: ArithImmShift =>
        s"${op.op}I\t${op.rd.show}, ${op.rs1.show}, ${op.shamt.show}"
      case op: JALR =>
        fansi.Color.Green(
          s"JALR\t${op.rd.show}, ${op.rs1.show}, ${op.dst.show}\t[${labelMap.lift(op.dst).getOrElse(UNKNOWN)}]"
        )
      case op: JAL =>
        fansi.Color.Magenta(
          s"JAL\t${op.rd.show}, ${op.dst.show} [${labelMap.lift(op.dst).getOrElse(UNKNOWN)}]"
        )
  ...
  implicit class IntPrinters(i: Int) {
    ...
    def binary: String = String
      .format("%" + 32 + "s", i.toBinaryString)
      .replace(' ', '0')
      .grouped(4)
      .map(x => x + "  ")
      .mkString
    def binary(n: Int): String =
      String.format("%" + n + "s", i.toBinaryString).replace(' ', '0')
  }

  def printProgram(vm: VM): String = {
    val withLabels: List[Either[(Addr, Op), (Label, Addr)]] =
      (vm.imem.toList.map(Left(_)) ::: vm.labelMap.toList.map(Right(_)))
        .sortBy {
          case Left(x)  => x._1.value.toDouble + 0.1;
          case Right(x) => x._2.value.toDouble
        }

    withLabels
      .map {
        case Left((addr, op)) =>
          s"$addr:\t\t${printInstruction(op, vm.labelMap)}"
  ...
  def printBinary(bin: Map[Addr, Int]): String = {
    bin.toList
      .sortBy(_._1.value)
      .map { case (addr, op) => s"$addr: ${op.hs}\t--\t${op.binary}" }
      .mkString("\n", "\n", "\n")
  }
  ...
  def printChiselLogEntry(event: (Addr, List[ChiselEvent])): fansi.Str = {
    ...
    val events      = log match {
      case (h: ChiselRegEvent) :: (t: ChiselMemWriteEvent) :: Nil =>
        h.show.padTo(25, ' ') ++ t.show
  ...
  def printVMtrace(
    ...
    val blocks: List[List[(ExecutionTraceEvent, Int)]] =
      LogParser.splitToBlocks(trace).zipWithIndexNested
    blocks.map { block =>
      val name =
        LogParser.guessBlockName(block.map(_._1.pc), program.labelMapReverse)
  ...
  def printSourceLine(addr: Addr, program: Program): fansi.Str =
    program.sourceMap.lift(addr).map(fansi.Str(_)).getOrElse(fansi.Str("???"))
  ...
  def printMergedTraces(
    ...
    /** For branch predicting processors we may end up with a spurious block
      * causing a transient desynchronization. Therefore it is necessary to have
      * a certain tolerance before calling a divergence.
      */
    ...
    def isSynchronized(
        desyncs: Int,
        traces: (List[ExecutionTraceEvent], List[CircuitTrace])
    ): Boolean = ???

    def helper(
        mt: (List[ExecutionTraceEvent], List[CircuitTrace])
    ): List[List[fansi.Str]] = mt match {
      ...
      /** The Block contains no more instructions performed by the VM. This
        * happens naturally since 5-stage pipelines will fetch spurious
        * instructions.
        */
      case (Nil, hC :: tC) => {
        val address        = hC._1
        ...
        val line = List(
          fansi.Color.LightBlue(address.show),
          chiselLogEntry,
          vmEntry,
          sourceLine
        )
      ...
      /** The Block contains no more instructions performed by the circuit. This
        * happens when the circuit takes a jump the VM does not. This is an
        * error *unless* the circuit has a branch predictor.
        *
        * If you want to you can splice the logs when this happens, but it's
      ...
      case (hVM :: tVM, Nil) => {
        ...
        val line = List(
          fansi.Color.LightRed(address.show),
          chiselLogEntry,
          vmEntry,
          sourceLine
        )
    ...
    def format(triplet: List[fansi.Str]): String = {
      ...
      triplet match {
        case address :: ch :: vm :: source :: Nil => {
          ...
          ((address ++ Str(":")).padTo(14, ' ') ++ vmLine ++ fansi.Str(
            "|    "
          ) ++ chiselLine ++ Str("|    ") ++ sourceLines).toString
        }
        case _ => ""
      }
    }
    ...
    val blockName =
      LogParser.guessBlockName(mt._1.map(_.pc), program.labelMapReverse)

    (fansi.Color.Yellow(s"$blockName").padTo(74, ' ').toString ++ "|".padTo(
      55,
      ' '
    ) ++ "|") :: helper(mt).map(format)
  }
  ...
  def printLogSideBySide(
      trace: List[ExecutionTraceEvent],
      chiselTrace: List[CircuitTrace],
      program: Program
  ): String = {
    import LogParser._
    val header =
      "ADDRESS   --   VM UPDATES                                                ---      DEVICE UNDER TEST UPDATES                     ---    CORRESPONDING SOURCE LINE\n"
    val traces =
      mergeTraces(trace, chiselTrace).map(x => printMergedTraces((x), program))
    "\n" + header + (traces
      .map(_.mkString("\n"))
      .mkString(
        "\n",
