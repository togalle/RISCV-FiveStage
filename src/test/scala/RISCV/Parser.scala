...
object Parser {
  ...
  def reg: Parser[Int]         =
    takeWhile(x => (x != ',' && x != ')')).map(lookupReg).attempt
  def sep: Parser[Unit] = many(whitespace) *> char(',') *> many(whitespace).void

  def branch: (Parser[Int], Parser[Int], Parser[String]) =
    (reg <~ sep, reg <~ sep, label)
  def branchZ: (Parser[Int], Parser[String]) = (reg <~ sep, label)

  def arith: (Parser[Int], Parser[Int], Parser[Int]) =
    (reg <~ sep, reg <~ sep, reg)
  def arithImm: (Parser[Int], Parser[Int], Parser[Int]) =
    (reg <~ sep, reg <~ sep, (hex | int))

  def stringWs(s: String): Parser[String] =
    many(whitespace) ~> string(s) <~ many1(whitespace)
  ...
  val singleInstruction: Parser[Op] = List(
    ...
    ////////////////////////////////////////////
    //// load/store
    stringWs("sw") ~> (reg <~ sep, (hex | int) <~ char('('), reg <~ char(')'))
      .mapN { case (rs2, offset, rs1) => SW(rs2, rs1, offset) },
    stringWs("lw") ~> (reg <~ sep, (hex | int) <~ char('('), reg <~ char(')'))
      .mapN { case (rd, offset, rs1) => LW(rd, rs1, offset) },
  ...
  val multipleInstructions: Parser[List[Op]] = List(
    stringWs("li") ~> (reg <~ sep, (hex | int).map(_.splitHiLo(20)))
      .mapN {
        case (rd, (hi, lo)) => {
          List(
    ...
    stringWs("lh") ~> (reg <~ sep, (hex | int) <~ char('('), reg <~ char(')'))
      .mapN {
        case (rd, offset, rs1) if (offset % 4 == 3) => {
          val placeHolder =
            if (rd == Reg("a0").value) Reg("a1").value else Reg("a0").value
          List(
  ...
  val setting = List(
    char('#') ~> string("regset") ~> many1(whitespace) ~> (
      reg.map(Reg.apply) <~ sep,
      hex | int
    ).mapN { REGSET.apply },
    char('#') ~> string("memset") ~> many1(whitespace) ~> (
      (hex | int).map(Addr.apply) <~ sep,
      hex | int
    ).mapN { MEMSET.apply }
  ...
  def parseProgram(
    ...
    /** The foldhelper represents a traversal through a RISC-V program.
      *
      * When it sees an op it records the operation and appends the source line
      * and its location. If it is in nopPad mode it will also insert NOPs
      * between the parsed ops. After appending ops the address counter is
      * bumbed accordingly
      *
      * When it sees a label destination it checks what the current addres
      * counter is at and creates a link to this address.
      *
      * When it sees a parse error it simply stores the error and keeps going,
      * allowing you to get every error (This works for an ASM program since
      * each line is independent)
      *
      * Lastly, when it sees a test setting it appends that test setting.
      *
      * The reason everything is treated all-in-one is to make it easier to
      * ensure that everything is parsed. If there were separate parsers for
      * ops, labels and settings it would be difficult to find out if errors
      * were simply of the wrong type or a legit error. This is not set in
      * stone, if you're re-architecturing the code maybe it's better to
      * separate parsers? Or maybe have multiple passes? Up to you!
      */
    case class FoldHelper(
      ...
      def addSettings(t: TestSetting): FoldHelper =
        copy(settings = t :: settings)
      def addErrors(t: String): FoldHelper  = copy(errors = t :: errors)
      def addLabelMap(t: Label): FoldHelper =
        copy(labelMap = labelMap + (t -> Addr(addrCount)))
      def addOps(t: List[SourceInfo[Op]]): FoldHelper = {
        if (testOptions.nopPadded) {
          copy(
            ops = t.flatMap(x =>
              (x :: List.fill(4)(
                SourceInfo("inserted NOP", NOP).widen[Op]
              )).reverse
            ) ::: ops,
      ...
      def program: Either[
        String,
        (List[TestSetting], List[SourceInfo[Op]], Map[Label, Addr])
      ] = {

        /** There are two possible ways for a program to successfully terminate,
          * either by explicitly executing a DONE instruction, or by returning
          * from the main method. In the latter case it is necessary to preload
          * the return address register such that the return instruction jumps
          * to a predetermined special done address
          */
        val hasDONEinstruction = ops.map(_.run._2).contains(DONE)
        val done               = copy(
          settings = REGSET(Reg("sp"), 1024) :: settings,
          ops = ops.reverse,
        ...
        val withReturnAddress = if (hasDONEinstruction) {
            ...
            REGSET(Reg("ra"), 0xeb1ceb1c) :: done.settings
          ) // now that's what I call EPIC

        Either
          .cond(
            errors.isEmpty,
            (withReturnAddress.settings, done.ops, labelMap),
            done.errors
          )
          .left
          .map(errors =>
    ...
    def foldHelper(acc: FoldHelper, program: List[(Int, String)]): FoldHelper =
      program match {
        case Nil                 => acc
        case (lineNo, line) :: t => {
          if (line.isEmpty)
            ...
            val next = all.parse(line).done.either match {
              case Left(parseError) =>
                acc.addErrors(f"$lineNo%3d" + s":$line\t$parseError")
              case Right(Left(setting))       => acc.addSettings(setting)
              case Right(Right(Right(label))) => acc.addLabelMap(label)
              case Right(Right(Left(ops)))    =>
                acc.addOps(
                  ops.map(op =>
                    SourceInfo(s"${lineNo.toString.padTo(3, ' ')}:\t$line", op)
                  )
                )
            }
            foldHelper(next, t)
    ...
    val results = foldHelper(
      FoldHelper(Nil, Nil, Map("zero" -> Addr(0)), Nil, 0),
      p.zipWithIndex.map(_.swap)
    )

    results.program.map { case (settings, ops, labelMap) =>
      Program(ops, settings, labelMap)
    }
  }
