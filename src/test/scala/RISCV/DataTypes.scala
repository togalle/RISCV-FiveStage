...
/** Types and extension methods go here. Maybe it's a litte arbitrary to put VM
  * types here, but op types in Ops.scala Maybe these could be separated to
  * somewhere else.
  */
object Data {
  type Label = String
  ...
  case class ExecutionTraceEvent(pc: Addr, event: ExecutionEvent*) {
    override def toString(): String = s"$pc: " + event.toList.mkString(", ")
  }
  ...
  case class ChiselRegEvent(pcAddr: Addr, reg: Reg, word: Int)
      extends ChiselEvent
  case class ChiselMemWriteEvent(pcAddr: Addr, memAddr: Addr, word: Int)
      extends ChiselEvent
  ...
  implicit class IntOps(i: Int) {
    ...
    def nBitsS: Int = i match {
      case i if (i < 0) =>
        (math.log(math.abs(i.toLong)) / math.log(2)).ceil.toInt + 1
      case i if (i == 0) => 0
      case i if (i > 0)  =>
        (math.log((i + 1).toLong) / math.log(2)).ceil.toInt + 1
    }

    /** Yes, a negative number technically has a unsigned size, but that depends
      * on integer width, so it is better left as an option
      */
    ...
    def splitHiLo(hiBits: Int): (Int, Int) = {
      ...
      say(
        s"split lo hi for $i with $hiBits high bits and got low: $lo, high: $hi"
      )
      (hi, lo)
    }
    ...
    def binary: Int = {
      s.reverse
        .foldLeft((0, 0)) {
          ...
          case ((acc, pow), char)                =>
            assert(false, "malformed binary conversion"); (0, 0)
        }
        ._1
    }
  }
  ...
  implicit class ListOps[A](xs: List[A]) {
    ...
    def splitAtPred(p: (A, A) => Boolean): List[List[A]] = {
      val splitPoints = xs.tail
        .foldLeft((1, List[Int](), xs.head)) {
    ...
    def showN(sep: String)(implicit ev: Fancy[A]): fansi.Str =
      xs.foldLeft(fansi.Str("")) { case (acc, a) =>
        acc ++ a.show ++ fansi.Str(sep)
      }
    def showN(sep1: String, sep2: String, sep3: String)(implicit
        ev: Fancy[A]
    ): fansi.Str =
      Str(sep1) ++ xs.foldLeft(fansi.Str("")) { case (acc, a) =>
        acc ++ a.show ++ fansi.Str(sep2)
      } ++ Str(sep3)
  ...
  implicit class NestedListOps[A](xs: List[List[A]]) {
    def zipWithIndexNested: List[List[(A, Int)]] = {
      ...
      (xs.map(_.zipWithIndex) zip startingPoints).map {
        case (withIndex, offset) =>
  ...
  /** Represents the result of parsing a program, with built in convenience
    * methods for running a VM and assembling the program.
    */
  case class Program(
    ...
    /** A convenient lookup for every instruction, allowing the test runner to
      * check what source line caused an error to happen.
      */
    val sourceMap: Map[Addr, String] =
      ops
        ...
        .zipWithIndex
        .map { case (info, idx) =>
          (Addr(idx * 4), info.map(_.trim).mkString(", "))
        }
        .toMap
    ...
    def machineCode: Either[String, Map[Addr, Int]] =
      imem.toList
        .sortBy(_._1.value)
        .map { case (addr, op) =>
          assembler.assembleOp(op, addr, labelMap).map(x => (addr, x))
        }
        .sequence
        .map(_.toMap)
        .left
        .map { case (error, addr) =>
          s"Assembler error: $error, corresponding to source:\n${sourceMap(addr)}"
        }

    /** Returns the binary code and the execution trace or an error for
      * convenient error checking.
      */
    def validate(
        maxSteps: Int
    ): Either[String, (Map[Addr, Int], ExecutionTrace[VM])] =
      machineCode.flatMap { binary =>
        val uk              = "UNKNOWN"
        val (finish, trace) = VM.run(maxSteps, vm)
        finish match {
          case Failed(s, addr) =>
            Left(
              s"VM failed with error $s at address $addr\nSource line:\n${sourceMap.lift(addr).getOrElse(uk)}"
            )
          case Timeout =>
            Left(
              s"VM timed out after $maxSteps steps. This should not happen with the supplied tests"
            )
