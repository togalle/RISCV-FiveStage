...
case class VM(
  ...
  /** The weird :_* syntax is simply a way to pass a list to a varArgs function.
    *
    * To see why, consider def printMany(a: Any*); printMany(List(1,2,3)) It is
    * now ambiguous if it should print "1, 2, 3" or List(1, 2, 3)
    *
    * This is disambiguated by appending :_*, thus printMany(List(1,2,3):_*) ==
    * printMany(1, 2, 3)
    */
  ...
  private def executeArithImmShift(op: ArithImmShift) = {
    val (regUpdate, nextRegs) =
      regs.arithImm(op.rd, op.rs1, op.shamt, op.op.run)
    val nextVM = this.copy(regs = nextRegs)
    Right(step(nextVM, regUpdate.toList: _*))
  }
  ...
  private def executeAUIPC(op: AUIPC) = {
    val (regUpdate, nextRegs) =
      regs + (op.rd -> (pc.value + (op.imm.value << 12)))
    val nextVM = this.copy(regs = nextRegs)
    Right(step(nextVM, regUpdate.toList: _*))
  }
  ...
  private def executeLW(op: LW) = dmem
    .read(Addr(regs.repr(op.rs1) + op.offset.value))
    .map { case (event, result) =>
  ...
  private def getOp: Either[Failed, Op] =
    imem
      ...
      .toRight(
        Failed(
          s"Attempted to fetch instruction at illegal address ${pc.show}",
          pc
        )
      )
}
...
object VM {
  ...
  val init =
    VM(DMem.empty, Map[Addr, Op](), Regs.empty, Addr(0), Map[Label, Addr]())
  ...
  def apply(
    ...
    val (dmem, regs) = settings.foldLeft((DMem.empty, Regs.empty)) {
      case ((dmem, regs), setting) =>
        setting match {
  ...
  def run(maxSteps: Int, vm: VM) = {
    def helper(
        state: ExecutionTrace[VM],
        step: Int
    ): (Finished, ExecutionTrace[VM]) = {
      if (step > 0) {
