  ...
  def profileBranching(testOptions: TestOptions): Boolean = {
    ...
    val testResults = for {
      ...
      case class Taken(from: Int, to: Int) extends BranchEvent {
        override def toString = s"Taken      ${from.hs}\t${to.hs}"
      }
      case class NotTaken(addr: Int) extends BranchEvent {
        override def toString = s"Not Taken  ${addr.hs}"
      }
      ...
      /** This is a sample profiler for a rather unrealistic branch predictor
        * which has an unlimited amount of slots
        */
      def OneBitInfiniteSlots(events: List[BranchEvent]): Int = {
        ...
        def helper(
            ...
            predictionTable: Map[Int, Boolean]
        ): Int = {
          events match {
            ...
            // called an if guard.
            case Taken(from, to) :: t if (predictionTable(from)) =>
              helper(t, predictionTable)
            case Taken(from, to) :: t if (!predictionTable(from)) =>
              1 + helper(t, predictionTable.updated(from, true))
            case NotTaken(addr) :: t if (predictionTable(addr)) =>
              1 + helper(t, predictionTable.updated(addr, false))
            case NotTaken(addr) :: t if (!predictionTable(addr)) =>
              helper(t, predictionTable)
            case Nil => 0
          }
      ...
      def twoBitPredictor(events: List[BranchEvent], slots: Int): Int = {
        ...
        case class nBitPredictor(
          ...
          def predict(pc: Int): Boolean = predictionRules(
            values(pc.getTag(slots))
          )
          ...
          override def toString =
            values.map(x => x.binary(2)).mkString("[", "][", "]")
        }
        ...
        events
          .foldLeft((0, initPredictor)) { case (((acc, bp), event)) =>
            println()
            ...
            say(bp)
            event match {
              case Taken(pc, _) if bp.predict(pc) => {
                say("HIT!"); (acc, bp.update(pc, true))
              }
              case Taken(pc, _) => {
                say("MISS!"); (acc + 1, bp.update(pc, true))
              }
              case NotTaken(pc) if !bp.predict(pc) => {
                say("HIT!"); (acc, bp.update(pc, false))
              }
              case NotTaken(pc) => {
                say("MISS!"); (acc + 1, bp.update(pc, false))
              }
            }
          }
          ._1
