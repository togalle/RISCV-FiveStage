...
This CPU uses an equivalent of a 1-bit branch predictor. However, it's not explicitly implemented using this bit. Instead, the BP is a list of target values, all initiated to 0. When the address is 0, it's equivalent to predicting not taken. When the value is not zero, it's an address and it's predicted as taken.
...
**Update** is done in EX. First it is checked if the current instruction is a branch or a jump, and then it updates the target prediction to either 0 or the appropriate address based on wether the branch was taken or not. Update happens one cycle later to ensure that updates occur only after the flush has completed and the pipeline has been restored to a valid state.
